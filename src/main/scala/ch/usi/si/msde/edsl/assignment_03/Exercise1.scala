...
object JsonDSL:
  ...
  given Conversion[Boolean, JsonBoolean] = (bool: Boolean) =>
    if (bool) then JsonTrue
    else JsonFalse

  given Conversion[Seq[JsonValue], JsonArray] = (seq: Seq[JsonValue]) =>
    JsonArray(seq.toList)
...
object HttpRequestDSL:
  ...
  case class POSTObject(request: HTTPObject):
    infix def withEntity(json: JsonObject): Future[Response] = {
      PostRequest(url = request.buildUrl(), rawJsonEntity = json.toString())
        .perform()
    }

  case class HTTPObject(
      baseUrl: List[String],
      URLScheme: URLScheme,
      queryString: Option[List[KeyValuePair]] = None
  ):
    infix def /(rest: String): HTTPObject = {
      HTTPObject(baseUrl ++ List(rest), URLScheme, queryString)
    }

    // Here we throw a RuntimeException because ? must be called before and only once
    // so if the List[keyValuePair] has a value its worng to call ? method.
    infix def ?(rest: KeyValuePair): HTTPObject = {
      ...
      HTTPObject(
        baseUrl = baseUrl,
        URLScheme = URLScheme,
        queryString = Some(List(rest))
      )
    }

    // If call & method but queryString has not a value yet we throw a RuntimeEception
    // because $ cannot be called before ?
    infix def &(rest: KeyValuePair): HTTPObject = {
      val toReturn: List[KeyValuePair] = queryString.getOrElse(
        throw new RuntimeException("Calling & before ? was called.")
      )
      HTTPObject(
        baseUrl = baseUrl,
        URLScheme = URLScheme,
        queryString = Some(toReturn ++ List(rest))
      )
    }
    ...
    def buildUrl(): URL = {
      ...
      URL(
        scheme = URLScheme,
        domainAndPath = DomainAndPath(baseUrl),
        queryString = query
      )
    }
